...
async function getUsuarios() {
    try {
        const respuesta = await fetch("http://localhost:3002/proyectos");/* fetch es una funcion que permite hacer peticiones a una url */
...
async function postUsuarios(proyectos) {
    try {
        const respuesta = await fetch("http://localhost:3002/proyectos", { /* Con post se debe abrir una llavem donde especifica el tipo de metodo y el header(siempre se mantiene igual) */
            method: "POST",
            headers: {
...
async function updateUser(id, proyectos) {
    try {
        const respuesta = await fetch(`http://localhost:3002/proyectos/${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json"
            },
...
async function deleteUsers(id) {
    try {
        const respuesta = await fetch(`http://localhost:3002/proyectos/${id}`, { /* Con put se debe abrir una llavem donde especifica el tipo de metodo y el header(siempre se mantiene igual) */
            method: "DELETE",
